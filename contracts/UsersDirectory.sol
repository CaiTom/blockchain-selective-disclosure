...
contract UsersDirectory is Ownable {
    ...
    uint256 count;
    mapping (address => bytes32[]) public belongTo; // One user could be in different group
    ...
    function addUser(bytes32 elementId, address user) public {
        ...
        belongTo[user].push(elementId); // Add user belongTo
    ...
    // Get the belongTo array
    function getUserGroup(address user) public view returns(bytes32[]) {
        return belongTo[user];
    }
